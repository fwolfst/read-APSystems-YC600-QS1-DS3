...
int decodePollAnswer(int which)
{
  ...
  float total_pwr = 0;
  if (!*inMessage)
    return POLLING_ERR_EMPTY_INMESSAGE; // if empty we return with erroCode 50
  ...
  {
    if (diagNose)
      ws.textAll("AF_DATA_REQUEST failed");
    return POLLING_ERR_AF_DATA_REQUEST_FAILED;
  }
  else if (strstr(messageToDecode, "FE03448000") == NULL) //  AF_DATA_CONFIRM the 00 byte = success
  {
    if (diagNose)
      ws.textAll("no AF_DATA_CONFIRM");
    return POLLING_ERR_AF_DATA_CONFIRM_MISSING;
  }
  else if (strstr(messageToDecode, "FE0345C4") == NULL) //  ZDO_SRC_RTG_IND
  {
    if (diagNose)
      ws.textAll("no route receipt");
    // return 12; // this command seems facultative
  }
  else if (strstr(messageToDecode, "4481") == NULL)
  {
    if (diagNose)
      ws.textAll("no AF_INCOMING_MSG");
  ...
  {
    if (diagNose)
      ws.textAll("ignoring, received " + String(messageToDecode));
  ...
  */
  ...
  if (t_extr < t_saved[which] || t_saved[which] == 0)
  ...
  for (int x = 0; x < 4; x++)
  {
    if (Inv_Prop[which].conPanels[x])
    { // is this panel connected ? otherwise skip
#ifndef TEST
      if (diagNose)
      {
        delay(100);
        ws.textAll(" * * * decoding panel " + String(x) + " * * * ");
      }
#endif
      ...
      {
        delay(100);
        ws.textAll(" * decoding panel " + String(x) + " * en_old " + String(en_old[x]));
      }
#endif
      ...
      {
        delay(100);
        ws.textAll("en_extr " + String(en_extr) + "  en_saved " + String(en_saved[which][x]));
      }
#endif
      ...
      if (diagNose)
      {
        ...
        {
          delay(100);
          ws.textAll("1st poll after inverter (re)start");
        }
        delay(100);
